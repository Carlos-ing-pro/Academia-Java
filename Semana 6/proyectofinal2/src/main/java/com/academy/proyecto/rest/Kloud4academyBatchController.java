...
public class Kloud4academyBatchController {
    ...
    private void commitBulkUpdateToMongoDB() {
        try {
            ...
            int count = 0;
            InventarioPc productData = null;
            List<InventarioPc> productInfoList = new ArrayList<>();

            for (String[] productRow : allData) {
                String[] productStringArry = new String[productRow.length];
                productData = new InventarioPc();
                count = 0;
                ...
                productData.setPcname(productStringArry[1]);
                productData.setPcmodelo(productStringArry[2]);
                productData.setZona(productStringArry[3]);
                productInfoList.add(productData);
            }

            BulkOperations bulkOps = mongoTemplate.bulkOps(BulkOperations.BulkMode.UNORDERED, InventarioPc.class);
